...
const polarUrl = "https://polar-chartreuse-silverfish.glitch.me/";

async function getPolar() {
    const response = await fetch(polarUrl);
...
const createtable = () => {
    ...
    favColor.innerText = "Favorite Color";

    const tr = document.createElement("tr");
    tr.append(id, name, city, favColor);
...
function populateTable(polar) {
    const tbody = document.querySelector('tbody');
    tbody.innerHTML = '';
  
    polar.forEach(polar => {
      const id = document.createElement('td');
      id.innerText = polar.id
   
      const name = polar.name.split(' ');
      ...
      city.innerText = polar.city;
  
      const favColor = document.createElement('td');
      favColor.innerText = polar.fav_color;
  
      const tr = document.createElement('tr')
      tr.append(id,  firstName, lastName, city, favColor);
      tbody.append(tr);
    });
  }
